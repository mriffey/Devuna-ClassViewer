...
!!! Edit Template Files List
!!! </summary>
winTemplateFiles PROCEDURE (pTemplateQ, sCaption)
...
Window               WINDOW('Template Files'),AT(,,198,178),DOUBLE,TILED,CENTER,GRAY,PALETTE(256),WALLPAPER('WALLPAPER.GIF')
                       LIST,AT(4,4,190,130),USE(?List),VSCROLL,ALRT(MouseLeft2),COLOR(COLOR:White),FLAT,FORMAT('60L(1)|FM~' & |
  'Name~@s63@?1020L(1)F~Path~S(255)@s255@'),FROM(Q),MSG('Browsing Queue')
                       BUTTON('&Insert'),AT(4,138,45,12),USE(?Insert),KEY(InsertKey),TIP('Add a new tempplate ' & |
  'file to the list')
                       BUTTON('&Delete'),AT(53,138,45,12),USE(?Delete),KEY(DeleteKey),TIP('Remove the currentl' & |
  'y highlighted file from the list')
                       BUTTON('Edit'),AT(102,138,45,12),USE(?Edit),HIDE
                       BUTTON('&Edit'),AT(149,138,45,12),USE(?EditSource:Button)
...
J   LONG
  CODE
  GlobalErrors.SetProcedureName('winTemplateFiles')
  ...
  SELF.SetAlerts()
  QEIP5:Fields.Init()
  QEIP5:Fields.AddPair(Q.szName,Q.szName)
  QEIP5:Fields.AddPair(Q.szPath,Q.szPath)
  QEIP5:PopupMgr.Init()
  ...
  Window{PROP:HLP} = '~WINTEMPLATEFILES'
